...
NSInteger const JDOpenUDIDErrorNone = 0;
NSInteger const JDOpenUDIDErrorOptedOut = 1;
NSInteger const JDOpenUDIDErrorCompromised = 2;


#define JDOpenUDIDLog(fmt, ...)

static NSString * JDOpenUDIDSessionCache = nil;
static NSString * const JDOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const JDOpenUDIDKey = @"OpenUDID";
static NSString * const JDOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const JDOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const JDOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const JDOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const JDOpenUDIDDomain = @"org.OpenUDID";
static NSString * const JDOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const JDOpenUDIDRedundancySlots = 100;


@interface JDOpenUDID (Private)
...
+ (void)_setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:JDOpenUDIDDomain];
#else
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forType:JDOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary *)_getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:JDOpenUDIDDomain];
#else
	id item = [pboard dataForType:JDOpenUDIDDomain];
#endif	
    if (item) {
        @try{
            item = [NSKeyedUnarchiver unarchiveObjectWithData:item];
        } @catch(NSException* e) {
            JDOpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
//
+ (NSString *)value {
    return [JDOpenUDID valueWithError:nil];
}


+ (NSString *)valueWithError:(NSError **)error {
    if (JDOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:JDOpenUDIDDomain
                                         code:JDOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
        return JDOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:JDOpenUDIDAppUIDKey];
    if(appUID == nil) {
    ...
    //
    id localDict = [defaults objectForKey:JDOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:JDOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:JDOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:JDOpenUDIDOOTSKey];
        optedOut = optedOutDate!=nil;
        JDOpenUDIDLog(@"localDict = %@",localDict);
    }
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:JDOpenUDIDRedundancySlots];
    for (int n = 0; n < JDOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",JDOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        UIPasteboard *slotPB = [UIPasteboard pasteboardWithName:slotPBid create:NO];
#else
        NSPasteboard* slotPB = [NSPasteboard pasteboardWithName:slotPBid];
#endif
        JDOpenUDIDLog(@"SlotPB name = %@",slotPBid);
        if (slotPB==nil) {
            // assign availableSlotPBid to be the first one available
            if (availableSlotPBid==nil) availableSlotPBid = slotPBid;
        } else {
            NSDictionary* dict = [JDOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:JDOpenUDIDKey];
            JDOpenUDIDLog(@"SlotPB dict = %@",dict);
            if (oudid==nil) {
            ...
            NSString* gid = [dict objectForKey:JDOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:JDOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;   
                }
            }
        }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [JDOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) { 
            ...
            [localDict setObject:openUDID forKey:JDOpenUDIDKey];
            [localDict setObject:appUID forKey:JDOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:JDOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:JDOpenUDIDTSKey];
    ...
    //
    JDOpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:JDOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict) {
            [JDOpenUDID _setDict:localDict forPasteboard:slotPB];
        }
    }
    ...
    if (localDict && saveLocalDictToDefaults) {
        [defaults setObject:localDict forKey:JDOpenUDIDKey];
    }
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:JDOpenUDIDDomain
                                                     code:JDOpenUDIDErrorOptedOut
        ...
        JDOpenUDIDSessionCache = [NSString stringWithFormat:@"%040x",0];
        return JDOpenUDIDSessionCache;
    }
    ...
    if (error != nil) {
        if (isCompromised) {
            *error = [NSError errorWithDomain:JDOpenUDIDDomain
                                         code:JDOpenUDIDErrorCompromised
    ...
    JDOpenUDIDSessionCache = openUDID ;
    return JDOpenUDIDSessionCache;
}
...
+ (void)setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:JDOpenUDIDKey];
    ...
    if (optOutValue) {
        [dict setObject:[NSDate date] forKey:JDOpenUDIDOOTSKey];
    } else {
        [dict removeObjectForKey:JDOpenUDIDOOTSKey];
    }

  	// store the dictionary locally
    [defaults setObject:dict forKey:JDOpenUDIDKey];
    
    JDOpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
    JDOpenUDIDSessionCache = nil;
}
